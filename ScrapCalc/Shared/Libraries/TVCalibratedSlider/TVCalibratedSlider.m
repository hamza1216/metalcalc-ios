...
@interface TVCalibratedSlider () <TVSliderDelegate>{
...
- (void)initializeSlider {
    ...
    _tvSliderView = [[TVSlider alloc] initWithFrame:CGRectMake(0, 0, self.frame.size.width,0)];
    _tvSliderView.continuous = NO;
    _tvSliderView.delegate = self;
...
#pragma mark - TVSliderDelegate

- (void)didChangeToAnotherValue:(NSInteger)value
{
...
- (void)setMaximumTrackImage:(NSString *)imageName withCapInsets:(UIEdgeInsets)capInsets forState:(UIControlState)state {
    ...
    else
    {
        ...
        [_tvSliderView setMaximumTrackImage:transparentImage forState:UIControlStateNormal];
    }
}
...
- (void)setMinimumTrackImage:(NSString *)imageName withCapInsets:(UIEdgeInsets)capInsets forState:(UIControlState)state {
    ...
    else
    {
        ...
        UIImage *transparentImage = UIGraphicsGetImageFromCurrentImageContext();
        UIGraphicsEndImageContext();
        [_tvSliderView setMinimumTrackImage:transparentImage forState:UIControlStateNormal];
    }
}
...
- (void)valueChanged:(id)sender {
    ...
    int calculatedValue = roundf(value);
